...
public:
	 ...
	 */
	ProgressBar(uint32_t total): total_(total){
		step_ = width_ / 10;
		progColors_ = RdYlGn_;
	};
	...
	std::map<uint32_t, std::string> readToGreen_ = {{0,bib::bashCT::addBGColor(196)},
			{1,bib::bashCT::addBGColor(199)},
			{2,bib::bashCT::addBGColor(201)},
			{3,bib::bashCT::addBGColor(129)},
			{4,bib::bashCT::addBGColor(57)},
			{5,bib::bashCT::addBGColor(27)},
			{6,bib::bashCT::addBGColor(39)},
			{7,bib::bashCT::addBGColor(45)},
			{8,bib::bashCT::addBGColor(50)},
			{9,bib::bashCT::addBGColor(46)}
	};
	std::map<uint32_t, std::string> RdYlGn_ = {{0,bib::bashCT::addBGColor(124)},
			{1,bib::bashCT::addBGColor(167)},
			{2,bib::bashCT::addBGColor(209)},
			{3,bib::bashCT::addBGColor(215)},
			{4,bib::bashCT::addBGColor(222)},
			{5,bib::bashCT::addBGColor(229)},
			{6,bib::bashCT::addBGColor(149)},
			{7,bib::bashCT::addBGColor(71)},
			{8,bib::bashCT::addBGColor(78)},
...
private:
	...
	bool progress(uint32_t current){
		if(current > total_){
			std::cerr << std::endl << "Error, " << current << "would progress further than total: " << total_ << std::endl;
		}
		current_ = current;
	...
	std::string getPerStr()const{
		...
		progSs << bib::bashCT::bold << static_cast<uint32_t>(currentProgress_ * 100) << "% " << bib::bashCT::reset << current_ ;
		return progSs.str();
	}
	uint32_t current_ = 0;
	uint32_t total_;
