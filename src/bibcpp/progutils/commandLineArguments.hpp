...
private:
	std::string& lowerCaseGetArguments(const std::string& str) {
		if (arguments_[strToLowerRet(str)] == "") {
			std::stringstream ss;
			ss << "Argument " << str << " requires an option but none was given"
					<< std::endl;
...
public:
	/**@brief Accessor to get the value associated with flag by case insensitive look up
	 ...
	 * @return Bool, true if flag is present or false if flag is absent, search is
	 * case insensitive
	 * @todo Set up for setting bool option to false with --no-FLAG or something
	 */
	 ...
	 */
	static std::map<std::string, std::string> convertCommandLineArguments(
			int argc, char* argv[]) {
