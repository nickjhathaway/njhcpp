...
#include "njhcpp/graphics/colorConstants.hpp"
#include "njhcpp/bashUtils.h"
#include "njhcpp/utils/vecUtils.hpp" //catVecs
namespace njh {
	...
	if(getHeader){
		...
		if(ansi){
			return std::vector<std::string>{estd::to_string(col.hexStr_), estd::to_string(col.hue_),
								...
								njh::bashCT::addBGColor(closeAnsi.first) + estd::to_string(closeAnsi.first) + njh::bashCT::reset,
								};
		}else{
			return std::vector<std::string>{estd::to_string(col.hexStr_), estd::to_string(col.hue_),
								estd::to_string(col.lSat_), estd::to_string(col.lum_),
 ...
 */
inline std::unordered_map<std::string, njh::color> getColorsForNames(
    const std::vector<std::string>& names, double sat, double lum) {
  std::unordered_map<std::string, njh::color> ret;
  uint32_t count = 0;
  std::vector<njh::color> colors = njh::evenHuesAll(sat, lum, names.size());
...
inline std::unordered_map<std::string, njh::color> getColorsForNames(
  ...
  std::unordered_map<std::string, njh::color> ret;
  uint32_t count = 0;
  std::vector<njh::color> colors = njh::getColsBetweenInc(hueStart, hueStop,
  		lumStart, lumStop,
  		satStart, satStop,
  		names.size());
  for (const auto& n : names) {
