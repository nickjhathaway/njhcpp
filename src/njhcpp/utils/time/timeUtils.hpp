...
#include "njhcpp/utils/utils.hpp" // printTableOrganized(), roundDecPlaces()
#include "njhcpp/utils/stringUtils.hpp"  //leftPadNumStr()
#include "njhcpp/utils/lexical_cast.hpp" //lexical_cast<T>()
#include "njhcpp/bashUtils.h" //njh::bashCT::boldRed()
...
inline double convertFormatedTimeToSecs(const std::string & str) {
	...
	for (const auto & t : toks) {
		...
		if (timeToks.front() == "yrs") {
			ret += njh::lexical_cast<double>(timeToks.back()) * 86400 * 365;
		} else if (timeToks.front() == "days") {
			ret += njh::lexical_cast<double>(timeToks.back()) * 86400;
		} else if (timeToks.front() == "hrs") {
			ret += njh::lexical_cast<double>(timeToks.back()) * 3600;
		} else if (timeToks.front() == "mins") {
			ret += njh::lexical_cast<double>(timeToks.back()) * 60;
		} else if (timeToks.front() == "secs") {
			ret += njh::lexical_cast<double>(timeToks.back());
		} else {
			throw std::runtime_error { njh::bashCT::boldRed(
